...
import pandas as pd
import base64
from PIL import Image # Embora PIL não seja usado diretamente para exibir a imagem base64, é bom manter para possíveis processamentos.
import datetime
...
# Não vamos mais armazenar o 'estoque' completo no session_state como um DataFrame
# Ele será lido do DB quando necessário.
    ...
    if st.session_state["logado"]:
                ...
                for i, row in produtos_para_exibir_viz_limitado.iterrows():
                    with cols_viz[col_idx_viz]:
                        ...
                        if 'data_compra' in row and pd.notna(row['data_compra']):
                            # A data já está como string YYYY-MM-DD
        ...
        if st.session_state["acesso_privilegiado"]:
            ...
            if df_estoque_atual.empty:
                ...
                nomes_produtos_ajuste = df_estoque_atual["nome_produto"].tolist()
                ...
                produto_para_ajustar_db = df_estoque_atual[df_estoque_atual["nome_produto"] == produto_a_ajustar_nome].iloc[0]
                idx_produto_ajustar_id = produto_para_ajustar_db["id"]
                ...
                if produto_para_ajustar_db is not None:
                    ...
                    if st.button("Aplicar Ajuste de Estoque", key="button_ajustar_estoque"):
                        ...
                        produto_data_temp = produto_para_ajustar_db.to_dict() # Converte para dict
                        produto_data_temp['quantidade'] = nova_quantidade # Atualiza a quantidade
                        
                        # Mapear chaves do DataFrame para as chaves esperadas pela função de atualização do BD
                        produto_data_db_update = {
                        ...
                        sucesso_ajuste = sf.atualizar_produto(NOME_DB, NOME_TABELA_PRODUTOS, idx_produto_ajustar_id, produto_data_db_update)
        ...
        colunas_db_ret, dados_db_ret = sf.selecionar_todos_produtos(NOME_DB, NOME_TABELA_PRODUTOS)
        df_estoque_retirada = pd.DataFrame(dados_db_ret, columns=colunas_db_ret)

        if df_estoque_retirada.empty:
            st.info("Nenhum produto disponível no estoque para retirada em lote.")
        else:
            produtos_disponiveis = df_estoque_retirada[df_estoque_retirada["quantidade"] > 0].copy()
            
            if produtos_disponiveis.empty:
                st.info("Nenhum produto com estoque disponível para retirada.")
                return

            opcoes_produtos_nomes = produtos_disponiveis["nome_produto"].tolist()

            produtos_selecionados_multiselect = st.multiselect(
                "Selecione os produtos para o carrinho:",
                options=opcoes_produtos_nomes,
                default=[
                    produto["nome_produto"] for id_prod, produto in produtos_disponiveis.iterrows()
                    if produto['id'] in st.session_state["carrinho"] and st.session_state["carrinho"][produto['id']] > 0
                ],
                key="multiselect_carrinho"
            )

            # Mapear nomes selecionados de volta para IDs do DB
            produtos_selecionados_ids = [
                produtos_disponiveis[produtos_disponiveis["nome_produto"] == nome]["id"].iloc[0]
                            ...
                            for produto_id, quantidade_retirar in st.session_state["carrinho"].items():
                                if quantidade_retirar > 0:
                                    ...
                                    if produto_no_estoque:
                                        ...
                                        # Mapear chaves do DB para as chaves esperadas pela função de atualização
                                        produto_data_db_update = {
                                            "Nome do Produto": produto_data_temp["nome_produto"],
                                            "Quantidade": produto_data_temp["quantidade"],
                                            "Preço de Compra (R$)": produto_data_temp["preco_compra"],
                                        ...
                                        sf.atualizar_produto(NOME_DB, NOME_TABELA_PRODUTOS, produto_id, produto_data_db_update)
                                        mensagens.append(f"{quantidade_retirar} unidades de '{produto_no_estoque['nome_produto']}' retiradas. Novo estoque: {nova_quantidade}.")
...
# A função main_app pode ser um ponto de entrada para o seu aplicativo Streamlit.
# Para manter a modularidade com o sistema de login, você pode chamá-la após o login.
# Aqui, vou chamá-la diretamente, mas em um aplicativo maior, ela seria chamada
# condicionalmente após a autenticação.
if __name__ == "__main__":
    # Nota: A lógica de login não está neste arquivo.
    # Você precisaria de um sistema de login separado que defina
    # st.session_state["logado"] e st.session_state["acesso_privilegiado"].
    # Para testar, você pode definir essas chaves como True no início do script.
    # Exemplo:
    # st.session_state["logado"] = True
    # st.session_state["acesso_privilegiado"] = True # Para testar como admin
